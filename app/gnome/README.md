...
- `launch` — `launch::activate(&Entry)` is the single dispatch point for "the user pressed Enter on this row". An exhaustive `match` routes entries:
  ...
  - `Entry::Command(cmd)` dispatches on `cmd.kind`. The three state-toggle kinds (`Minimize`, `ToggleMaximize`, `ToggleFullscreen`) route to the corresponding by-id D-Bus wrappers in `windows` (`windows::minimize_window(id)`, `windows::toggle_maximize_window(id)`, `windows::toggle_fullscreen_window(id)`). The six geometry kinds run through `lofi_core::compute_geometry(kind, &cmd.work_area, cmd.current_frame)`; the resulting `(x, y, w, h)` is fed to `windows::move_resize_window(cmd.target_window_id, x, y, w, h)`. `target_window_id`, `work_area`, and `current_frame` were all captured at gather time by `commands::gather_commands`, so the activation path is one D-Bus call with no further reads.
