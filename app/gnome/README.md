...
- `power` — system-level power commands (Lock, Suspend, Restart, Shutdown). Two free functions:
  ...
  - The module uses lower-level `Proxy::new` + `call_method` rather than generating four `#[zbus::proxy]` traits. Each call is one line of business logic — a generated trait per service would be more noise than signal, especially since three of the four traits would be one-method one-call wrappers we never reuse.
