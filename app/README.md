...
- `macos/` — macOS-specific code (experimental). Swift + AppKit on top of `lofi-core` as a `staticlib`, built by Bazel (`rules_rust` + `rules_swift` + `rules_apple`). First slice shows a borderless `NSPanel` listing `.app` bundles under `/Applications` and `~/Applications`. Same data-flow pattern as GNOME: the platform layer discovers and pushes into the Rust-owned entry list. See `macos/README.md`.
