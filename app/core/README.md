...
- `search` **ranks by fuzzy score**: matching entries are returned sorted by descending score (best match first), so a query like `"Code"` puts `"Visual Studio Code"` above an app that only matches via scattered letters. Pure subsequence matching is too permissive on its own to surface in a useful order; the score is what does that.

`search` is the pure score-only primitive — it has no notion of recency. The launcher does not call `search` directly for the displayed list; both platforms call `ranking::rank` (below), which reuses `matcher`'s filtering core but layers the two-tier MRU + prefix order on top. `search` remains as a focused, MRU-free entry point (and is what the matcher's own tests exercise).

The "haystack" — the text we match against — is built per-variant by an exhaustive `match` on `Entry` inside a private `haystack` function. For `Entry::Application` it is `"{name} {desktop_id}"`, so typing either the display name or the desktop id works — with one carve-out: the reverse-DNS prefix is stripped from `desktop_id` before it enters the haystack, leaving only the final app-specific segment. Both macOS bundle ids and GNOME desktop ids encode the vendor as a reverse-DNS prefix (`com.apple.Safari`, `org.mozilla.FirefoxDeveloperEdition`, `org.gnome.Nautilus.desktop`), and leaving any of it in turns short or scattered queries into noise — `"m"` would fuzzy-match every `com.apple.*` id via the `m` in `com.`, and `"maxi"` would fuzzy-match `org.mozilla.FirefoxDeveloperEdition` via the `m`/`a` of `mozilla` plus the `x`/`i` of `Firefox`. Stripping back to the last segment kills both noise floors while keeping the app-specific portion (`Safari`, `FirefoxDeveloperEdition`, `Nautilus`) searchable. A `.desktop` suffix is removed first so GNOME ids collapse to their app segment too instead of all becoming the trailing `desktop`. The trade-off is that vendor-only searches (`"google"` against `com.google.Chrome`) no longer match via the id — those have to come from the entry's `name` field. For `Entry::Window` it is `"{title} {app_name}"` when `app_name` is `Some`, and just `title` when it is `None`. The practical consequence is that typing an app name (e.g. `"firefox"`) matches both the Firefox application entry and every open Firefox window in the same gather. For `Entry::Workspace` the haystack is `name` alone — no second field worth concatenating, and the default `"Workspace N"` label already makes `"work"`, `"2"`, and `"workspace 2"` all match the right row; a custom workspace-naming extension flows its label through unchanged. For `Entry::Command` the haystack is `kind.display_name()` alone — the kind id (e.g. `"center_half"`) is a persistence detail, not a user-visible string, and matching on it would let typos in old MRU rows surface as ghost matches. For `Entry::PowerCommand` the haystack is also `kind.display_name()` alone, for the same reason — the snake_case ids (`"lock_session"`, etc.) are persistence-only, and matching on the display name (`"Lock"`, `"Suspend"`, ...) is the user-facing surface. For `Entry::WorkspaceCommand` the haystack is the entry's `name` field (e.g. `"Move to workspace 3"`, `"Move to next workspace"`), not the `as_id` persistence string — so typing `"move"`, `"workspace"`, `"3"`, `"next"`, or `"previous"` all match the right rows, while the snake_case id (`"move_to_workspace_2"`) stays out of the haystack. Future `Entry` variants force this function to be updated (no `_` arm).
...
Alongside `search`, the `matcher` module carries two `pub(crate)` primitives that `ranking::rank` builds on:

- `score(entry, tokens, matcher) -> Option<i64>` — the single-entry filter/score function. Returns `None` if any token fails to fuzzy-match the haystack (intersection semantics), else the summed per-token score. `search` and `rank` share it so they always agree on what matches.
- `is_prefix_match(entry, tokens) -> bool` — the prefix signal. A token is a *word prefix* when it is a case-insensitive prefix of at least one whitespace-separated word of the **same** `haystack()` that `score` uses (so the two never disagree about what text an entry is matched against). `"c"` and `"goo"` are word prefixes of `"Google Chrome"`; `"hrome"` (mid-word) and `"gc"` (scattered) are not. Multi-token follows the same AND rule as `score`: the entry is a prefix match only when *every* token word-prefixes *some* word. Empty `tokens` is vacuously true. Case folding uses `to_lowercase()` (not `eq_ignore_ascii_case`) so non-ASCII names fold correctly.

### `ranking::rank` and `ranking::mru_rank_map`

This is the **single source of truth for ranking**, shared by both platforms and re-exported at the crate root (`lofi_core::rank`, `lofi_core::mru_rank_map`). Ranking is core's responsibility, not the platform layers': the macOS FFI (`EntryList::recompute_filter`) and the GNOME UI (`populate_list`) both delegate here so the two frontends produce byte-for-byte identical order from the same inputs. A platform's job is reduced to *gather entries + supply an MRU recency map*, then *display the ordered result and handle the selected entry* — it never re-implements the matching/sort policy.

```rust
pub fn rank(
    entries: &[Entry],
    query: &str,
    mru: &HashMap<EntryRef, usize>,
) -> Vec<usize>

pub fn mru_rank_map(index: &[EntryRef]) -> HashMap<EntryRef, usize>
```

`mru_rank_map` builds the recency-rank map both callers pass to `rank`: it takes an `MruStore::read_all()`-ordered slice (most-recent-first) and assigns rank 0 to the first element, 1 to the next, and so on. Sharing this helper is what makes "rank" mean the same thing on both the GNOME `populate_list` path and the FFI `apply_mru` path.

`rank` returns the surviving entries' indices into `entries`, in display order:

1. **Filter.** The query is split on whitespace into tokens, and each entry is kept iff `matcher::score` returns `Some` (every token must fuzzy-match the haystack — the same intersection semantics as `search`). An empty / whitespace-only query yields no tokens, so `score` returns `Some(0)` for every entry: empty query is a passthrough that includes everything.
2. **Two tiers.** Survivors split by MRU membership. **Tier 1** is entries the user has launched (their `EntryRef` is a key in `mru`); **Tier 2** is never-used entries.
3. **Tier 1 order** is a stable sort by `(prefix_bucket, recency_rank)`, where `prefix_bucket` is `0` when the entry is a word-prefix match (`matcher::is_prefix_match`) and `1` otherwise, and `recency_rank` is the `mru` map value (0 = most recent). So among the user's known apps, prefix matches float above non-prefix matches, and within a bucket more-recent beats less-recent.
4. **Tier 2 order** is a stable sort by descending fuzzy score — the best textual match in the never-used set comes first (this is `search`'s order).
5. **Concatenate** Tier 1 then Tier 2.

The key design decision: **the prefix signal reorders ONLY within the MRU tier.** A never-used prefix match can never outrank any MRU-known entry, even a non-prefix one. Recency is the dominant signal — "the apps you actually use bubble to the top" — and prefix is a tiebreak *within* that set, sharpening the order of your known apps as you type without letting a never-launched app jump the queue just because its name happens to start with your query. Both sorts are stable, so genuine ties (same bucket + rank, or equal score) preserve input order.

The empty-query passthrough falls out of the same code path rather than being special-cased: empty tokens make `score` return `Some(0)` (everything included) and `is_prefix_match` vacuously true (every Tier 1 entry lands in `prefix_bucket` 0, leaving them in pure recency order), while Tier 2 entries all score 0 and keep input order. Net result: MRU-known entries by recency, then never-used entries in input order.

The practical consequence while typing: MRU-known rows are query-independent in *recency* terms but can still shuffle between the two prefix sub-buckets as the query changes (a row drops below the others the moment your query stops being a word-prefix of it); never-used rows can reorder as the scores shift. The selected row stays predictable for the apps you use most.
...
SQLite-backed activation history. The store is the launcher's persistent record of which `EntryRef`s the user has activated, with a recency timestamp per ref. The launcher reads it once at startup, feeds the recency ordering into `ranking::rank` (via `mru_rank_map`) where it is the dominant ranking signal for the displayed list, and writes back synchronously on every activation. See the `ranking::rank` section above for exactly how recency, prefix, and fuzzy score combine.
...
Window entries follow the same Swift-produces / Rust-holds shape as Application — pushed into the Rust list as `Entry::Window` via `lofi_entries_push_window`. The `Window::id` (a `CGWindowID` on macOS) is the cross-platform stable identifier — analogous to `Application::desktop_id` — and is the field Swift reads back via `lofi_entries_get_window_id` to drive activation. Because the Rust `Window` struct deliberately doesn't carry platform-specific fields like the owning process's PID (see `Window` above for the rationale), Swift maintains a side-table `[CGWindowID: (pid_t, title)]` populated at push time. The Rust core stays platform-clean; Swift looks up the PID it already knew at gather time when it's ready to call `AXUIElementPerformAction`. **The macOS frontend doesn't currently push Window entries** — the per-window switcher is disabled there (see `app/macos/README.md` gotchas 13-14), so the `push_window` FFI is exercised only by `tests/ffi.rs` today. The macOS `WindowDiscovery` uses AX (`NSWorkspace.frontmostApplication` + `kAXFocusedWindowAttribute`) to find the foreground window for window-action commands; that path uses `WindowCommands.CommandTarget` rather than `Entry::Window` and never crosses the FFI as a Window entry.
...
- `lofi_entries_get_category(list, idx)` — returns one of six stable English strings (`"Application"`, `"Window"`, `"Workspace"`, `"Command"`, `"Power"`, `"WorkspaceCommand"`). Chosen over exposing the `EntryKind` discriminant because a stable string is cheaper across the FFI boundary than threading an enum value plus a Swift-side translation table; localization, if needed, can come later as a UI override. `"Power"` (rather than `"PowerCommand"`) matches the label GNOME's `kind_to_str` renders for the same `EntryKind`, so both platforms display the same category text. `"WorkspaceCommand"` is present for match-exhaustiveness only — `Entry::WorkspaceCommand` is a GNOME-only kind the macOS frontend never pushes into an `EntryList`, so this string is never actually returned across the FFI in practice.
...
- `lofi_entries_apply_mru(list, store)` — reads every row from the MRU store (`MruStore::read_all`), builds a recency-rank map keyed by `EntryRef` (rank 0 = most recent) via `ranking::mru_rank_map`, stores it on the list, clears every `CString` cache, and recomputes the active filter. Returns true on success, false on null arguments or a `read_all` failure (degraded mode: leave the rank map untouched). Called once after the push loop and before showing the panel. **It does NOT reorder the underlying `Vec<Entry>`.** The rank map is the only state it changes; `recompute_filter` then delegates to `ranking::rank`, which reads that map to produce the two-tier display order (MRU-known matches first, in recency-then-prefix order; never-launched matches after, by descending fuzzy score — see the `ranking::rank` section above) on demand. Keeping the vec fixed is what makes the per-accessor caches — keyed on the *underlying* index — stable across an `apply_mru`: no entry ever moves between positions, so a cached `CString` can never point at a different entry's text. (The cache still clears on every mutation as a belt-and-braces measure, but the keying is now robust by construction.)
...
- `lofi_entries_push_application(list, name, bundle_id, icon, is_running) -> bool` gained an `is_running` boolean parameter. Pass `true` when the app's process is up at gather time; the value is stored as `Application::is_running` and read back through `lofi_entries_get_is_running`. The macOS `AppDelegate.summonPanel` derives the boolean from a one-pass scan of `NSWorkspace.shared.runningApplications` (collecting `bundleIdentifier`s) — no TCC grant required, so the indicator works in degraded apps-only mode. On the GNOME side this FFI is not used — the Rust core constructs `Application` directly, so the `is_running` field is set in lockstep with `recent_window_id` in `lofi-gnome::main`.
...
`lofi_entries_set_query(list, query)` recomputes the filter. A null or empty `query` is a passthrough (every entry in rank order); a non-null UTF-8 `query` is whitespace-tokenized and intersected against each entry's per-variant haystack. `recompute_filter` delegates entirely to `ranking::rank` — the same single ranking implementation the GNOME frontend calls — so filtering, the two-tier MRU/score order, the in-MRU prefix sub-ordering, and the empty-query passthrough all live in core, not in the FFI layer. It differs from bare `matcher::search` (pure descending-score order) by overlaying that MRU + prefix policy; see the `ranking::rank` section above for the exact algorithm. After `set_query` returns, `lofi_entries_len` and every `get_*` accessor read through the filter — Swift's table view sees a contiguous, post-filter list and doesn't need to know which underlying indices survived.

Note that `filter` is now `Some(...)` after every recompute (the empty-query passthrough yields `Some(all-indices-in-rank-order)` rather than `None`); it is `None` only on a fresh or just-cleared list, before any recompute has run.
...
`apply_mru` joins the invalidation list too, though its reason is now weaker than `set_query`'s. `apply_mru` no longer reorders the underlying `entries: Vec<Entry>` — it only rebuilds the recency-rank map and recomputes the filter — so the caches, which key off the *underlying* index, would in fact survive the rank-map change intact (no entry moves between positions). But `apply_mru` also recomputes `filter`, which is the same kind of membership/order change `set_query` makes, and the borrow contract is stated against *filtered* indices the caller passed to `get_*`. Clearing all four caches on `apply_mru` keeps the contract uniform with `set_query` — "no mutating call between a `get_*` and the read of its bytes" — rather than carving out a special "this mutation happens to be cache-safe" exception. It is cheaper and more obviously correct than a per-index validity-tracking scheme.
...
Bazel only sees what it builds, which is `core` (the `gnome` crate is Linux-only — gtk4/libadwaita — and has no Bazel target). And unlike Cargo, Bazel needs every test file spelled out as a target, with only files belonging to a declared target reachable by the clippy/rustfmt gates. The two integration test files each get a `rust_test` target — `:ffi_test` and `:mru_test` — and the inline `#[cfg(test)]` modules under `src/**/*.rs` (in `matcher.rs`, `lib.rs`, `commands.rs`, `mru.rs`) get one more — `:lofi_core_lib_test`, a `rust_test` with `crate = ":lofi_core_rlib"` that rebuilds the library with `--test --cfg test` so every inline `#[test]` is visible to Bazel. Without that target the inline cases would run under `cargo test` but be invisible to Bazel — and so escape the clippy gate, which only lints what it builds. The gates list the library plus every test target:

- `:clippy` (`rust_clippy`) runs clippy over `:lofi_core_rlib`, `:lofi_core_lib_test`, `:ffi_test`, and `:mru_test` with warnings promoted to errors (rules_rust's default for an explicit `rust_clippy` target). `:lofi_core_lib_test` is listed in addition to `:lofi_core_rlib` because clippy on the rlib alone runs without `--cfg test` and so does not lint `#[cfg(test)]` modules — the inline test code is only linted via the `--test` rebuild that target wraps. `testonly` because three of its deps are tests, and `:lofi_core` (the staticlib) is omitted because it compiles the same sources with the same features as the rlib — identical diagnostics, double the work.
- `:rustfmt` (`rustfmt_test`) is a non-mutating format check over the same four targets. Listing the library covers `src/**`; the two integration test targets cover `tests/ffi.rs` and `tests/mru.rs`. `:lofi_core_lib_test` reuses the library's `srcs` and contributes no new files, but is listed for symmetry with the clippy gate.
