...
- `search` **ranks by fuzzy score**: matching entries are returned sorted by descending score (best match first), so a query like `"Code"` puts `"Visual Studio Code"` above an app that only matches via scattered letters. Pure subsequence matching is too permissive on its own to surface in a useful order; the score is what does that. On top of this score order the launcher overlays a two-tier MRU policy — see `mru` below and the callers `EntryList::recompute_filter` (macOS) and `populate_list` (GNOME): entries the user has activated before are held in recency order ahead of the rest, and only the never-launched matches keep `search`'s score order. The practical consequence while typing: MRU-known rows stay put (their order is query-independent), while never-launched rows can reorder as the query narrows and the scores shift.
...
SQLite-backed activation history. The store is the launcher's persistent record of which `EntryRef`s the user has activated, with a recency timestamp per ref. The launcher reads it once at startup, uses recency as the **primary** sort key for the displayed list — with the matcher's fuzzy score as the secondary key for entries the user has never launched — and writes back synchronously on every activation.
...
- `lofi_entries_apply_mru(list, store)` — reorders the underlying `Vec<Entry>` by recency from the MRU store, clears every `CString` cache, and recomputes the active filter against the freshly reordered list. Returns true on success, false on null arguments or a `read_all` failure (degraded mode: leave order untouched). Called once after the push loop and before showing the panel. It records how many leading entries are MRU-known (`mru_count`) so that on any subsequent `set_query`, `recompute_filter` applies the two-tier order: MRU-known matches stay in recency order, and only the never-launched matches are sorted by descending fuzzy score.
...
`lofi_entries_set_query(list, query)` recomputes the filter. A null `query` clears the filter (identity passthrough); a non-null UTF-8 `query` is whitespace-tokenized and intersected against each entry's per-variant haystack using the same filtering and scoring core as `matcher::search` (`matcher::score`). It differs from bare `matcher::search` in ordering: `search` returns pure descending-score order, whereas `recompute_filter` additionally overlays the two-tier MRU policy described under `apply_mru` above (MRU-known matches in recency order first, never-launched matches by score after). After `set_query` returns, `lofi_entries_len` and every `get_*` accessor read through the filter — Swift's table view sees a contiguous, post-filter list and doesn't need to know which underlying indices survived.
