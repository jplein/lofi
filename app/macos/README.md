...
11. **CGWindow titles and AX titles disagree, in two distinct ways.** Bridging from a `CGWindowID` to an `AXUIElement` requires matching `kCGWindowName` against `kAXTitleAttribute`, and naive string equality fails on Chrome (and any app that does either of the things below) — selecting the row "renders" the row but never raises the window:
    - **CGWindow truncates long titles with a Unicode ellipsis (`…`, U+2026)**; AX keeps the full text. CGWindow `"Where has Safari gone, and why…"` never `==` AX `"Where has Safari gone, and why are macOS updates larger for Apple silicon? – The Eclectic Light Company"`.
    - **AX often appends the owning app's name as a suffix.** Chrome is the canonical case: CGWindow `"Hacker News"` vs AX `"Hacker News - Google Chrome"`. Some Electron apps do the same; AppKit apps mostly don't.

    `WindowActivation.titleMatches(...)` handles both with one rule: take the pre-ellipsis prefix of the CGWindow title and accept any AX title that *starts with* that prefix. `hasPrefix("Hacker News")` swallows the `" - Google Chrome"` suffix for free; pre-ellipsis prefix matching handles the truncation case. The remaining first-match-wins ambiguity (two windows whose long-enough prefixes happen to be identical) is the same hazard exact-titled siblings have ("Untitled — TextEdit" three times) — the robust fix uses the private `_AXUIElementGetWindow` API to disambiguate against the `CGWindowID` we already have in `windowAux`. Out of scope for this slice; the failure mode is "wrong window of N similar ones raised", not "raise fails".
